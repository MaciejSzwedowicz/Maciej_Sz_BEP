...
def normalize_scalar(val):
    if isinstance(val, str):            # to remove this function
...
class DrugRegistry:
    ...
    def get_or_create(self, conn, drug):
        ...
        if not name:
            return None  # skip drugs with missing name

        # Case-sensitive literal key
        if name in self.name_to_id:
            return self.name_to_id[name]

        # Assign new ID and insert into catalog
        ...
        # downstream tables will still use this drug_id
        ...
        if isinstance(openfda, dict):
            ...
            if any(v not in [None, "", "null"] for v in field_data.values()):
                ...
                insert_with_fields(conn, "drug_openfda", list(field_data.keys()), field_data)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--db", default="sql/openfda_base_redesigned_gpt_fullrun.db")
