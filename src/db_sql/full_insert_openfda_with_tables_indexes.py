...
# Drop tables if they exist (for fresh run)
tables = [
    "safety_reports", "patients", "reactions", "drugs", "activesubstances", "openfda",
    "openfda_metadata_extended", "patient_info_extended", "report_metadata",
    "patient_deaths", "drug_dosage_details"
]
for table in tables:
...
CREATE TABLE patients (
    ...
    FOREIGN KEY(safetyreportid) REFERENCES safety_reports(safetyreportid)
);
...
CREATE TABLE drug_dosage_details (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    drug_id INTEGER,
    drugbatchnumb TEXT,
    drugdosageform TEXT,
...
CREATE TABLE openfda_metadata_extended (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    openfda_id INTEGER,
    nui TEXT,
    package_ndc TEXT,
    ...
    spl_id TEXT,
    spl_set_id TEXT,
    FOREIGN KEY(openfda_id) REFERENCES openfda(id)
);
...
CREATE TABLE patient_deaths (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    patient_id INTEGER,
    dateofdeath TEXT,
    dateofdeathformat TEXT,
    autopsyperformed TEXT,
    FOREIGN KEY(patient_id) REFERENCES patients(id)
);

-- Indexes
CREATE INDEX idx_patients_safetyreportid ON patients(safetyreportid);
CREATE INDEX idx_reactions_patient_id ON reactions(safetyreportid);
CREATE INDEX idx_drugs_patient_id ON drugs(safetyreportid);
CREATE INDEX idx_activesubstances_drug_id ON activesubstances(drug_id);
CREATE INDEX idx_openfda_drug_id ON openfda(drug_id);
CREATE INDEX idx_openfda_meta_openfda_id ON openfda_metadata_extended(openfda_id);
CREATE INDEX idx_patient_deaths_patient_id ON patient_deaths(patient_id);
CREATE INDEX idx_report_metadata_safetyreportid ON report_metadata(safetyreportid);
CREATE INDEX idx_drug_dosage_details_drug_id ON drug_dosage_details(drug_id);
CREATE INDEX idx_drugs_medicinalproduct ON drugs(medicinalproduct);
CREATE INDEX idx_reactions_reactionmeddrapt ON reactions(reactionmeddrapt);
CREATE INDEX idx_safety_reports_receivedate ON safety_reports(receivedate);
CREATE INDEX idx_safety_reports_transmissiondate ON safety_reports(transmissiondate);
CREATE INDEX idx_safety_reports_receiptdate ON safety_reports(receiptdate);
CREATE INDEX idx_drugs_patient_product ON drugs(safetyreportid, medicinalproduct);
""")
conn.commit()

# End timer (just for table creation step)
end_time = time.time()
print(f"✅ Tables and indexes created in {end_time - start_time:.2f} seconds.")
